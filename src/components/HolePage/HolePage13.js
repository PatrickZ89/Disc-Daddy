...
class HolePage13 extends Component {
    ...
    nextHole = () => {
        ...
        for (let i = 0; i < this.props.playerReducer.length; i++) {
            ...
            gameData={strokes:this.state[i], score: score, playerID:playerID, gameID: this.props.gameIDReducer, hole:'hole_13'}
            
            this.props.dispatch({ type: 'POST_SCORE', payload: gameData })
        };
        this.props.dispatch({ type: 'SET_SCORE', payload: this.state })
        let path = `hole-page14`;
        this.props.history.push(path)
    }
...
export default connect(mapStateToProps)(HolePage13);